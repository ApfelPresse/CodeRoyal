...
cdef class GameManager:
    cdef public int max_turns
    cdef public int league_level
    cdef bint solo
    cdef List[Player] players
    cdef List[Player] active_players
...
cdef class Referee:
    cdef public List[Obstacle] obstacles
    cdef public GameManager game_manager
    cdef public bint end_game
    cdef int turn
    cdef bint towers
    cdef bint giants
    cdef bint mines
    cdef int fixed_income
    cdef int obstacle_count
    cdef int queen_hp
    ...
    cdef List[Obstacle] get_buildings_of_player(self, player):
        buildings = []
    ...
    cdef List[Unit] all_units(self):
        units = []
        for player in self.game_manager.players:
            units.extend(player.all_units())
        return units

    cdef all_entities(self):
        ent = []
        ent.extend(self.obstacles)
        ent.extend(self.all_units())
        return ent

    cdef schedule_building(self, player: Player, obs: Obstacle, struc_type: str,
                           obstacles_attempted_to_build_upon: list,
    ...
    cdef player_loop(self, obstacles_attempted_to_build_upon, scheduled_buildings):
...
cdef class Obstacle(FieldObject):
    cdef Optional[Structure] structure
    cdef public int obstacle_id
    cdef int max_mine_size
    cdef int gold
    cdef dict params
    cdef int obstacle_tile_id
    cdef int area

    def __init__(self, int max_mine_size, int initial_gold, int initial_radius, Vector2 initial_location,
                 int obstacle_id):
        super().__init__()
...
cdef class Queen(Unit):
    ...
    cdef move_towards(self, Vector2 target):
    ...
    cdef bint is_in_range_of(self, obs: Obstacle):
...
cdef class Player:
    ...
    cdef print_obstacle_per_turn(self, obstacle: Obstacle):
...
cdef class Vector2:
    ...
    cdef snap_to_integers(self):
        return Vector2(round(self.x), round(self.y))

    cdef float distance_to(self, Vector2 other):
        return np.sqrt((self.x - other.x) ** 2 + (self.y - other.y) ** 2)

    cdef check_if_not_smaller_zero(self):
        if self.x < 0:
            self.x = 0
        if self.y < 0:
            self.y = 0
    ...
    cdef Vector2 clamp_within(self, min_x: float, max_x: float, min_y: float, max_y: float):
        if self.x < min_x:
            nx = min_x
        elif self.x > max_x:
            nx = max_x
    ...
    cdef resized_to(self, new_length):
        return self.normalized() * new_length

    cdef normalized(self):
        length = math.sqrt(self.length_squared())
        if length < 1e-6:
            return Vector2(1, 0)
        return Vector2(self.x / length, self.y / length)

    cdef towards(self, Vector2 other, float max_distance):
